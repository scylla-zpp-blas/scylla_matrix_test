...
class COO : public multiplicator<T> {
    ...
    void submit_block(std::vector<matrix_value<T>>& block, size_t block_id, size_t matrix_id) {
        ...
        query << "INSERT INTO zpp.vectors (block_id, matrix_id, vals) "
    ...
    size_t get_block_index_for_cell(std::pair<size_t, size_t> pos) {
        ...
        size_t second = div_up(pos.first, _block_size);
        return div_up(dimension, _block_size) * first + second;
    }
public:
    ...
    void multiply() {
        ...
        for (size_t i = 1; i <= blocks_high; i++) {
            for (size_t j = 1; j <= blocks_wide; j++) {
                size_t block_id = i * blocks_wide + j;
...
int main(int argc, char* argv[]) {
    ...
    try {
        conn = std::make_shared<connector>("127.17.0.2");
    ...
    multiplicator_instance.load_matrix(sparse_matrix_value_generator<float>(dimension, dimension, 100, 0, factory));
    multiplicator_instance.load_matrix(sparse_matrix_value_generator<float>(dimension, dimension, 80, 1, factory));
